package project.msc.syntaxtree;

import project.msc.nodes.*;
import project.msc.nodes.binarynodes.*;
...
public class Parser {
    ...
    public Node parse() throws Exception {
        ...
        try {
            ...
            }else if(tokens.get(1).getKey() == TokenCheck.ADDITION && tokens.size() ==3){
            ...
            else if(tokens.get(1).getKey() == TokenCheck.SUBTRACTION && tokens.size() ==3){
                ExpressionNode left = new LiteralNode(tokens.get(0).getValue());
                ExpressionNode right = new LiteralNode(tokens.get(2).getValue());
                ast = new SubtractionNode( left,"-", right);

            }
            else if(tokens.get(1).getKey() == TokenCheck.DIVIDE && tokens.size() ==3){
                ExpressionNode left = new LiteralNode(tokens.get(0).getValue());
                ExpressionNode right = new LiteralNode(tokens.get(2).getValue());
                ast = new DivideNode(left,"/",right);

            }
            else if(tokens.get(1).getKey() == TokenCheck.MULTIPLICATION && tokens.size() ==3){
                ExpressionNode left = new LiteralNode(tokens.get(0).getValue());
                ExpressionNode right = new LiteralNode(tokens.get(2).getValue());
